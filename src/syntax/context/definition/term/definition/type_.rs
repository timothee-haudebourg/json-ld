...
impl<T> From<crate::syntax::NotAKeyword<T>> for InvalidTypeKeyword<T> {
	fn from(crate::syntax::NotAKeyword(t): crate::syntax::NotAKeyword<T>) -> Self {
		Self::NotAKeyword(t)
	}
}
...
impl<'a> From<&'a TermType> for ExpandableRef<'a> {
	fn from(d: &'a TermType) -> Self {
		match d {
			TermType::Term(t) => Self::String(t),
			TermType::Keyword(k) => Self::Keyword((*k).into()),
		}
	}
}
