...
use crate::syntax::{try_from::TryFromJson, Container, ErrorCode, Keyword, Nullable};
use iref::IriRefBuf;
use json_syntax::{code_map::Mapped, CodeMap};
...
impl From<crate::syntax::Unexpected> for InvalidContext {
	fn from(crate::syntax::Unexpected(u, e): crate::syntax::Unexpected) -> Self {
		Self::Unexpected(u, e)
	}
}
	...
	fn try_from_json_at(
		...
		match value {
			...
			json_syntax::Value::Object(o) => {
				...
				for Mapped((Mapped(key, key_offset), Mapped(value, value_offset)), _) in
					o.into_iter_mapped(code_map, offset)
				{
					match Keyword::try_from(key.as_str()) {
						Ok(Keyword::Id) => {
							def.id =
								Some(Nullable::try_from_json_at(value, code_map, value_offset)?)
						}
