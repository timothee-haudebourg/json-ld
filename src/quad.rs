...
use crate::{Id, Reference, Object, Node, Indexed, ExpandedDocument, object};
...
enum QuadsFrame<'a, J: JsonHash, T: Id> {
	IndexedObjectSet(Option<&'a Reference<T>>, std::collections::hash_set::Iter<'a, Indexed<Object<J, T>>>),
	IndexedObjectSlice(Option<&'a Reference<T>>, std::slice::Iter<'a, Indexed<Object<J, T>>>),

	NodeTypes(Option<&'a Reference<T>>, &'a Reference<T>, std::slice::Iter<'a, Reference<T>>),
	NodeProperties(Option<&'a Reference<T>>, &'a Reference<T>, object::node::properties::Iter<'a, J, T>),
	NodeReverseProperties(Option<&'a Reference<T>>, &'a Node<J, T>, object::node::reverse_properties::Iter<'a, J, T>),
...
impl<'a, J: JsonHash, T: Id> Quads<'a, J, T> {
	fn push_object(&mut self, graph: Option<&'a Reference<T>>, object: &'a Indexed<Object<J, T>>) {
		match object.inner() {
			Object::Node(node) => {
				self.push_node(graph, node)
			},
			Object::List(objects) => {
				self.stack.push(QuadsFrame::IndexedObjectSlice(graph, objects.iter()))
			},
			Object::Value(_) => ()
		}
	}
	...
	fn push_node(&mut self, graph: Option<&'a Reference<T>>, node: &'a Node<J, T>) {
		if let Some(id) = node.id() {
			self.stack.push(QuadsFrame::NodeReverseProperties(graph, node, node.reverse_properties().iter()));
			self.stack.push(QuadsFrame::NodeProperties(graph, id, node.properties().iter()));
			self.stack.push(QuadsFrame::NodeTypes(graph, id, node.types().iter()));
		}
	}
}
		...
		while let Some(last) = self.stack.last_mut() {
			match last {
				...
				QuadsFrame::NodeTypes(graph, subject, types) => {
					...
					match types.next() {
						Some(ty) => {
							return Some(QuadRef(graph, subject, PropertyRef::Type, ObjectRef::Ref(ty)))
						},
						None => {
							self.stack.pop();
						}
				...
				QuadsFrame::NodeProperties(graph, subject, properties) => {
					...
					match properties.next() {
						Some((property, objects)) => {
							self.stack.push(QuadsFrame::NodePropertyObjects(graph, subject, property, objects.iter()))
						},
						None => {
							self.stack.pop();
						}
				...
				QuadsFrame::NodeReverseProperties(graph, object, reverse_properties) => {
					...
					match reverse_properties.next() {
						Some((property, subjects)) => {
							self.stack.push(QuadsFrame::NodeReversePropertySubjects(graph, object, property, subjects.iter()))
						},
						None => {
							self.stack.pop();
						}
				...
				QuadsFrame::NodePropertyObjects(graph, subject, property, objects) => {
					...
					match objects.next() {
						Some(object) => {
							self.push_object(graph, object);
							return Some(QuadRef(graph, subject, PropertyRef::Ref(property), ObjectRef::Object(object)))
						},
						None => {
							self.stack.pop();
						}
					...
					match subjects.next() {
						Some(subject) => {
							...
							if let Some(id) = subject.id() {
								return Some(QuadRef(graph, id, PropertyRef::Ref(property), ObjectRef::Node(object)))
							}
						},
						None => {
							self.stack.pop();
