...
impl<'a> Expander<'a> {
	...
	pub async fn expand_node(
		...
		if self.active_property.is_none()
			|| self.active_property.is_some_and(|p| p == Keyword::Graph)
		{
	...
	async fn expand_node_entries(
		...
		for ExpandedEntry(key, expanded_key, value) in expanded_entries {
			let entry_loc = location.object_value(key);
			match expanded_key {
				Term::Null => (),
					...
					match expanded_property {
						...
						Keyword::Id => {
							...
							if let Some(str_value) = value.as_str() {
								...
								// false for vocab.
								result.id = node_id_of_term(self.active_context.expand_iri(
									// &mut env,
									// active_context,
						...
						Keyword::Type => {
							...
							for ty in value {
								if let Some(str_ty) = ty.as_str() {
									if let Ok(ty) = type_scoped_context
										.expand_iri(
											// &mut env,
											// type_scoped_context,
						...
						Keyword::Graph => {
							...
							let expanded_value = Box::pin(
								self.with_active_property(Some("@graph"))
									.expand_element(env, value, false, entry_loc),
							)
							.await?;
						...
						Keyword::Included => {
							...
							let expanded_value = Box::pin(
								self.with_active_property(Some("@included"))
									.expand_element(env, value, false, entry_loc),
							)
							.await?;
						...
						Keyword::Reverse => {
							...
							if let Some(value) = value.as_object() {
								let mut reverse_entries: Vec<_> = value.iter().collect();

								if self.options.ordered {
									reverse_entries.sort_by_key(|(_, key)| *key)
								}

								for (reverse_key, reverse_value) in reverse_entries {
									let rev_entry_loc =
										entry_loc.object_value(reverse_key.as_str());
									match self.active_context.expand_iri(
										...
										Term::Id(reverse_prop)
											if reverse_prop.as_str().contains(':')
												|| self.options.policy
													== ExpansionPolicy::Relaxed =>
										{
											let reverse_expanded_value = Box::pin(
												self.with_active_property(Some(
													reverse_key.as_ref(),
												))
												.expand_element(
													env,
											...
											let is_double_reversed =
												if let Some(reverse_key_definition) =
													self.active_context.get(reverse_key.as_str())
												{
						...
						Keyword::Nest => {
							...
							for nested_value in value {
								...
								let property_scoped_context =
									match self.active_context.get(nesting_key) {
										Some(definition) => {
								...
								if let Some(nested_value) = nested_value.as_object() {
									let mut nested_entries: Vec<EntryRef> = Vec::new();
									...
									if self.options.ordered {
										nested_entries.sort_by_key(|(_, key)| *key);
									}
				...
				Term::Id(prop)
					if prop.as_str().contains(':')
						|| self.options.policy == ExpansionPolicy::Relaxed =>
				{
					let mut container_mapping = Container::new();

					let key_definition = self.active_context.get(key);
					...
					let mut expanded_value = if is_json {
						...
						match value.as_object() {
							Some(value) if container_mapping.contains(ContainerItem::Language) => {
								...
								let mut direction = self.active_context.default_base_direction();
								...
								let mut language_entries: Vec<EntryRef> =
								...
								if self.options.ordered {
									language_entries.sort_by_key(|(_, key)| *key);
								}
							...
							Some(value)
								if container_mapping.contains(ContainerItem::Index)
									|| container_mapping.contains(ContainerItem::Type)
									|| container_mapping.contains(ContainerItem::Id) =>
							{
								...
								let mut entries: Vec<EntryRef> = Vec::with_capacity(value.len());
								...
								for (index, index_value) in entries {
									...
									// expanding index.
									let expanded_index = match self.active_context.expand_iri(
										// &mut env,
										// active_context,
									...
									let expanded_index_value = Box::pin(
										self.with_active_context_and_property(
											&map_context,
											Some(key),
										)
										.expand_element(
									...
									for mut item in expanded_index_value {
										...
										if expanded_index.is_some() {
												...
												&& index_key != "@index"
											{
												...
												let re_expanded_index = self
													.with_active_property(Some(index_key))
													.expand_literal(
														|w| env.warn(w),
														index.as_str().into(),
														entry_loc,
											...
											} else if container_mapping.contains(ContainerItem::Id)
												&& item.id().is_none()
											{
												// Otherwise, if container mapping includes
												...
												if let Object::Node(ref mut node) = *item {
													node.id = node_id_of_term(
														self.active_context.expand_iri(
															// &mut env,
															// active_context,
															Nullable::Some(index.as_str().into()),
															true,
							...
							_ => {
								...
								Box::pin(
									self.with_active_property(Some(key))
										.expand_element(env, value, false, entry_loc),
								)
								.await?
							}
					...
					if container_mapping.contains(ContainerItem::Graph)
						&& !container_mapping.contains(ContainerItem::Id)
						&& !container_mapping.contains(ContainerItem::Index)
					{
