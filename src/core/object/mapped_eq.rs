...
pub trait MappedEq<T: ?Sized = Self> {
	...
	fn mapped_eq(&self, other: &T, f: impl Clone + Fn(&BlankId) -> &BlankId) -> bool;
}
	...
	for<'a> &'a Self: IntoIterator<Item = &'a Self::Item>,
{
	...
	fn unordered_mapped_eq(&self, other: &Self, f: impl Clone + Fn(&BlankId) -> &BlankId) -> bool {
...
impl MappedEq for super::Object {
	fn mapped_eq(&self, other: &Self, f: impl Clone + Fn(&BlankId) -> &BlankId) -> bool {
		match (self, other) {
...
fn opt_mapped_eq<A: MappedEq>(
	a: Option<&A>,
	b: Option<&A>,
	f: impl Clone + Fn(&BlankId) -> &BlankId,
) -> bool {
	match (a, b) {
		(Some(a), Some(b)) => a.mapped_eq(b, f),
		(None, None) => true,
...
impl MappedEq for super::node::Properties {
	fn mapped_eq(&self, other: &Self, f: impl Clone + Fn(&BlankId) -> &BlankId) -> bool {
		if self.len() == other.len() {
...
impl MappedEq for super::node::ReverseProperties {
	fn mapped_eq(&self, other: &Self, f: impl Clone + Fn(&BlankId) -> &BlankId) -> bool {
		if self.len() == other.len() {
			let other_vec: Vec<_> = other.iter().collect();
			let mut selected = Vec::new();
