...
async fn compact_property_list<T: Sync + Send + Id, C: ContextMut<T>, L: Loader>(
	list: &[Indexed<Object<T>>],
	expanded_index: Option<&str>,
	nest_result: &mut json::object::Object,
	container: Container,
	as_array: bool,
...
async fn compact_property_graph<T: Sync + Send + Id, C: ContextMut<T>, L: Loader>(
	node: &Node<T>,
	expanded_index: Option<&str>,
	nest_result: &mut json::object::Object,
	container: Container,
...
fn select_nest_result<'a, T: Id, C: ContextMut<T>>(
	result: &'a mut json::object::Object,
	active_context: Inversible<T, &C>,
	item_active_property: &str,
	compact_arrays: bool,
) -> Result<(&'a mut json::object::Object, Container, bool), Error> {
...
pub async fn compact_property<
	'a,
	T: 'a + Sync + Send + Id,
	N: 'a + object::Any<T> + Sync + Send,
	O: IntoIterator<Item = &'a Indexed<N>>,
	C: ContextMut<T>,
	L: Loader,
>(
	result: &mut json::object::Object,
							...
							&& expanded_item.is_value()
						{
							...
							if index_key == "@index" {
								...
								let (map_key, remaining_values) = match &mut compacted_item {
									JsonValue::Object(map) => {
										match map.remove(container_key.as_str().unwrap()) {
							...
							let (map_key, remaining_values) = match &mut compacted_item {
								JsonValue::Object(map) => match map
									.remove(container_key.as_str().unwrap())
								{
									Some(value) => match value {
										JsonValue::Short(_) | JsonValue::String(_) => {
