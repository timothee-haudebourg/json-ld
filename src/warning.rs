...
impl fmt::Display for Warning {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match self {
			...
			Self::BlankNodeIdProperty(id) => write!(f, "blank node identifier `{}` is used as property", id),
			Self::KeywordLikeTerm(term) => write!(f, "term `{}` has the form of a keyword", term),
			Self::KeywordLikeValue(value) => write!(f, "value `{}` has the form of a keyword", value),
			Self::MalformedLanguageTag(tag, e) => write!(f, "malformed language tag `{}`: {}", tag, e),
