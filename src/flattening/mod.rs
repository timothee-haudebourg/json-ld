...
use crate::{id, ExpandedDocument, Id, Indexed, Node, Object};
...
impl<J: JsonHash + JsonClone, T: Id> ExpandedDocument<J, T> {
	pub fn flatten_ordered<G: id::Generator<T>>(
		self,
		generator: G,
	) -> Result<Vec<Indexed<Node<J, T>>>, ConflictingIndexes<T>> {
